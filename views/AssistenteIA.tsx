...
const AssistenteIA: React.FC<AssistenteIAProps> = ({ produtos, vendasHoje, contas }) => {
  ...
  const handleAsk = async (text?: string) => {
    ...
    const context = {
      produtos: produtos.map(p => ({ nome: p.name, qtd: p.stock_quantity, alerta: p.min_stock_alert })),
      totalVendasHoje: vendasHoje.reduce((acc, v) => acc + v.amount, 0),
      contasPendentes: contas.filter(c => c.status !== 'paid').length
    };
  ...
  return (
    <PageTransition>
      <div className="flex flex-col h-[calc(100vh-140px)] bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden relative">
